...
int PCSX::Memory::psxMemInit() {
    ...
    if (g_psxMemRLUT == nullptr || g_psxMemWLUT == nullptr || g_psxM == nullptr || g_psxP == nullptr || g_psxH == nullptr) {
        PCSX::g_system->message("%s", _("Error allocating memory!"));
        return -1;
    }

    // MemR
    for (auto i = 0; i < 0x80; i++) g_psxMemRLUT[i + 0x0000] = (uint8_t *)&g_psxM[(i & 0x1f) << 16];
    ...
    for (auto i = 0; i < 0x08; i++) g_psxMemRLUT[i + 0x1fc0] = (uint8_t *)&g_psxR[i << 16];
    ...
    // MemW
    for (auto i = 0; i < 0x80; i++) g_psxMemWLUT[i + 0x0000] = (uint8_t *)&g_psxM[(i & 0x1f) << 16];
...
uint8_t PCSX::Memory::psxMemRead8(uint32_t mem) {
    ...
    if (pointer != nullptr) {
        ...
        return *(uint8_t *)(pointer + (mem & 0xffff));
    } else {
        if (page == 0x1f80 || page == 0x9f80 || page == 0xbf80) { // handle the scratcpad/IO page
            if ((mem & 0xffff) < 0x400)
                return psxHu8(mem);
            else
...
uint16_t PCSX::Memory::psxMemRead16(uint32_t mem) {
    ...
    if (pointer != nullptr) {
        ...
        return SWAP_LEu16(*(uint16_t *)(pointer + (mem & 0xffff)));
    } else {
        if (page == 0x1f80 || page == 0x9f80 || page == 0xbf80) { // handle the scratcpad/IO page
            if ((mem & 0xffff) < 0x400)
                return psxHu16(mem);
...
void PCSX::Memory::psxMemWrite8(uint32_t mem, uint8_t value) {
    ...
    if (pointer != nullptr) {
        ...
        *(uint8_t *)(pointer + (mem & 0xffff)) = value;
...
void PCSX::Memory::psxMemWrite16(uint32_t mem, uint16_t value) {
    ...
    if (pointer != nullptr) {
        ...
        *(uint16_t *)(pointer + (mem & 0xffff)) = SWAP_LEu16(value);    
        PCSX::g_emulator->m_psxCpu->Clear((mem & (~3)), 1);
...
void PCSX::Memory::psxMemWrite32(uint32_t mem, uint32_t value) {
    ...
    if (pointer != nullptr) {
        ...
        *(uint32_t *)(pointer + (mem & 0xffff)) = SWAP_LEu32(value);    
...
void *PCSX::Memory::psxMemPointer(uint32_t mem) {
    ...
    const auto pointer = (char *)(g_psxMemWLUT[page]);
    if (pointer != nullptr)
        return (void *)(pointer + (mem & 0xffff));
