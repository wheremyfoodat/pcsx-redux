...
class X86DynaRecCPU : public PCSX::R3000Acpu {
	...
	const uint32_t STATUS_OFFSET = (uintptr_t) &m_psxRegs.CP0.r[12] - (uintptr_t) &m_psxRegs; // the offset of the SR reg in the register struct
	...
	const uint32_t HI_OFFSET = (uintptr_t) &m_psxRegs.GPR.n.hi - (uintptr_t) &m_psxRegs; // the offset of the hi reg in the register struct
	const uint32_t LO_OFFSET = (uintptr_t) &m_psxRegs.GPR.n.lo - (uintptr_t) &m_psxRegs; // the offset of the lo reg in the register struct
...
// backend methods
  private:
	...
	const int REC_MEMORY_SIZE = 32 * MEGABYTE;  // how big our x64 code buffer is. This is 32MB vs the 32-bit JIT's 8MB,
	...
	void recompileBlock (uintptr_t*& blockPointer) {
		...
		gen.align (32); // align buffer to 32 bytes
	...
	void recSLL() {
		...
		if (isConst(_Rt_)) // if $rt is const
			markConst (_Rd_, registers[_Rt_].val << _Sa_); // $rd = $rs << sa
	
		else { // nothing is constant
			allocateReg (_Rt_);
			allocateReg (_Rd_);
			gen.mov (registers[_Rd_].allocatedReg, registers[_Rt_].allocatedReg); // $rd = $rt
			gen.shl (registers[_Rd_].allocatedReg, _Sa_); // $rd <<= shift amount
		}
	}
	...
	void recSRL() {
		...
		if (isConst(_Rt_)) // if $rt is const
			...
			allocateReg (_Rd_);
			gen.mov (registers[_Rd_].allocatedReg, registers[_Rt_].allocatedReg); // $rd = $rt
			gen.shr (registers[_Rd_].allocatedReg, _Sa_); // $rd >>= shift amount 
		}
	}
	...
	void recJAL() {
		markConst (31, recPC + 4); // store the return address in $ra and mark as const. Note: The return address is 8 bytes after the JAL
	...
	void recREGIMM() {
		...
		switch (_Rt_) {
			case 0: recBLTZ <false>(); break; // BLTZ
			case 0x10: recBLTZ <true>(); break; // BLTZAL
			default: printf ("Unknown REGIMM mode: %d", _Rt_); exit (1);
		}
	}
	...
	void recBLTZ() {
		...
		if constexpr (link) // if link, set $ra to return address
			markConst (31, recPC + 4); // the address is always linked, even if the branch is not taken!

		if (isConst(_Rs_)) { // if $rs is constant
			if ((int32_t) registers[_Rs_].val < 0) // if $rs < 0
	...
	void recSLTU() {
        ...
        allocateReg (_Rd_);
		if (isConst(_Rt_)) { // Rt is constant
			allocateReg (_Rs_);
			gen.cmp (registers[_Rs_].allocatedReg, registers[_Rt_].val); // compare $rs and $rt
	...
	void recSLTI() {
		...
		if (isConst(_Rs_)) // if both Rs and Rt are const
			markConst (_Rt_, (int32_t) registers[_Rs_].val < (int32_t) _Imm_); // set $rt to 1 if $rt < val (signed), else set to 0
	...
	/*
		...
		this is x86 code for:
		status = (status & ~0xF) | ((status >> 2) & 0xF)
		And fixes up the COP0 status register's interrupt and kernel/user mode flags
	*/
	void recRFE() {
		gen.mov (eax, dword [rbp + STATUS_OFFSET]); //SR in eax
		gen.mov (ecx, eax); // copy SR to ecx
		gen.and_ (eax, ~0xF); // clear low nibble
		gen.shr (ecx, 2); // ecx >>= 2;
		gen.and_ (ecx, 0xF); // fetch low nibble of ecx after shifting (aka old bits 6:2)
		gen.or_ (eax, ecx); // copy bits 6:2 to 3:0
		gen.mov (dword [rbp + STATUS_OFFSET], eax); // write back status

		printf("Check out the testSWInt shit from the original JIT\n");
	}

	void recMFHI() {
		allocateReg (_Rd_);
		gen.mov (registers[_Rd_].allocatedReg, dword [rbp + HI_OFFSET]); // load hi into $rd
	}

	void recMTHI() {
		allocateReg (_Rs_);
		gen.mov (dword [rbp + HI_OFFSET], registers[_Rs_].allocatedReg); // load $rs into hi
	}

	void recMFLO() {
		allocateReg (_Rd_);
		gen.mov (registers[_Rd_].allocatedReg, dword [rbp + LO_OFFSET]); // load lo into $rds
	}

	void recMTLO() {
		allocateReg (_Rs_);
		gen.mov (dword [rbp + LO_OFFSET], registers[_Rs_].allocatedReg); // load $rs into lo
	}
